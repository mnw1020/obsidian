module.exports = async (params) => {
    ...
    if (!entries.length) {
        new Notice("У этой книги пока нет записей чтения.");
        return;
    }
    ...
    const labels = sorted.map(entry => {
        ...
        return `Чтение #${entry.number}${entry.date ? ` - ${displayDate(entry.date)}` : ""}${rating}`;
    });
    const selected = await quickAddApi.suggester(labels, sorted, "Какое чтение редактировать?");
    if (!selected) return;
    ...
    const inputs = [
        {
            id: "date",
            label: `Дата чтения #${selected.number}`,
            type: "text",
            defaultValue: selected.date,
            placeholder: "YYYY-MM-DD, можно YYYY-MM или YYYY"
        }
    ];
    ...
    if (fiction) {
        inputs.push({
            ...
            type: "number",
            optional: true,
            defaultValue: selected.rating === null ? "" : String(selected.rating),
    ...
    const target = entries.find(entry => entry.number === selected.number);
    if (!target) {
        new Notice("Не удалось найти выбранное чтение.");
        return;
    }
    target.date = date;
    ...
    if (issues.length) {
        new Notice(`${title}: чтение #${selected.number} обновлено. ⚠️ ${issues.join("; ")}. Запусти «Проверить библиотеку».`, 9000);
    } else {
        new Notice(`Чтение #${selected.number} обновлено`);
    }
