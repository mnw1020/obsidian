---
Выбрано: Hannah Spearritt (Ханна Спирритт)
---
...
if (!selected) {
    ...
    dv.table(["Произведение", "Тип", "Релиз", "Моя оценка", "IMDb", "Франшиза"], rows.map(p => [
        p.file.link,
...
<!-- Старое представление Bases оставлено внутри комментария для совместимости со старыми копиями. -->
<!--
```base
filters:
  and:
