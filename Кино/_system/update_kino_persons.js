...
module.exports = async function updateKinoPersons(params) {
    ...
    try {
        ...
        for (const entry of unknown.values()) {
            for (const field of entry.fields) {
                if (personDisplay(field, entry.text, aliases) === entry.text) unresolved++;
            }
        }
        notice.hide?.();
        new ob.Notice(
            `Готово: карточек ${records.length}, обновлено ${updated}, новых соответствий ${newPairs.size}, без однозначной пары ${unresolved}.`,
            10000
        );
    } catch (_) {
...
function emptyCache() {
    return { Актеры: {}, Режисер: {}, Проверено: { Актеры: {}, Режисер: {} } };
}
...
async function requestJson(ob, base, params) {
        ...
        try {
            const response = await Promise.race([
                ob.requestUrl({ url: url.href, method: "GET", throw: false,
                    headers: { Accept: "application/sparql-results+json", "User-Agent": "ObsidianKinoteka/2.0" } }),
            ...
            if (status !== 200 || !response?.json) return null;
