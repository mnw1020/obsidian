...
function isWatched(x,index){
    if(x?.watched||x?.local?.watched)return true;
...
function combineRecommendationLists(base,extras,showWatched,watchedItems) {
    const seen=new Set(),result=[];
    for(const x of [...base,...extras]) {
        if(!showWatched&&isWatched(x,watchedItems))continue;
...
async function main(){
    ...
    const draw=()=>{
        const visible=combineRecommendationLists(all,(tmdbCheckbox.checked?tmdbRows:[]),checkbox.checked,watchedItems);
        renderTable(ui,visible);
        const hidden=all.filter(x=>isWatched(x,watchedItems)).length;
