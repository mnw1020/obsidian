...
const KP_API_KEY_STORAGE = "kino.recommendations.kinopoiskApiKey";
let KP_API_KEY = "18560e74-f0bf-4ca5-9efc-5a9ebb547268";
try { KP_API_KEY = String(localStorage.getItem(KP_API_KEY_STORAGE) || "").trim(); } catch(_) {}
const KP_DIAG = { api:"не настроен", direct:"не проверялся", fallback:"не проверялся", lastSimilar:"", lastDetail:"" };
...
async function kpApiGet(path, timeout=6000){
    ...
    try{
        ...
        if(status===401||status===403) KP_DIAG.api=`HTTP ${status}: ключ неверный/нет доступа`;
        else if(status===429) KP_DIAG.api="HTTP 429: лимит запросов";
        else KP_DIAG.api=`HTTP ${status||0}`;
        return {ok:false,status,data:r?.json||null,reason:"http"};
    }catch(e){ KP_DIAG.api=`ошибка: ${String(e?.message||e).slice(0,80)}`; return {ok:false,status:0,data:null,reason:"exception"}; }
}
...
async function getSimilarKp(kpId,sourceCache){
    ...
    if(KP_API_KEY){
        ...
        if(api.ok){
            ...
            return {items:[],source:"kp-api-empty",diag:"KP API: HTTP 200, похожих 0"};
        }
    }
    ...
    const diag=[KP_API_KEY?`KP API ${KP_DIAG.api}`:"KP API: ключ не настроен",`страница КП: ${KP_DIAG.direct}`,KP_DIAG.fallback].join("; ");
...
async function getKpDetail(kpId,fallbackTitle,sourceCache){
    ...
    // Primary when configured: KP API by exact Kinopoisk ID.
    if(KP_API_KEY){
        const api=await kpApiGet(`/v2.2/films/${kpId}`,6500);
        if(api.ok){
            const d=parseKpUnofficialDetail(api.data,kpId,fallbackTitle);
            if(d){KP_DIAG.lastDetail=`KP API: ${d.description?"описание есть":"описания нет"}`;sourceCache.details[kpId]={at:Date.now(),data:d,source:"kp-api"};return d;}
        }
    }

    // Second: actual Kinopoisk page.
    const kp=await getKinopoiskPage(kpId);
    if(kp.ok){
        const d=parseKinopoiskFilmPage(kp.text,kpId,fallbackTitle);
        if(d){KP_DIAG.lastDetail=`страница КП: ${d.description?"описание есть":"описания нет"}`;sourceCache.details[kpId]={at:Date.now(),data:d,source:"kinopoisk"};return d;}
    }
    ...
    if(r.ok){
        ...
        if(d){KP_DIAG.lastDetail=`Movie Planner: ${d.description?"описание есть":"описания нет"}`;sourceCache.details[kpId]={at:Date.now(),data:d,source:"movie-planner"};return d;}
    }
...
function addKpApiControl(ui){
    const label=ui.actions.createEl("span",{text:KP_API_KEY?"КП API: подключен":"КП API: не настроен"});label.style.opacity="0.8";
    const btn=ui.actions.createEl("button",{text:KP_API_KEY?"Сменить API-ключ":"Настроить API-ключ"});
    btn.onclick=()=>{
        const v=window.prompt("API-ключ kinopoiskapiunofficial.tech. Ключ хранится только в localStorage Obsidian и не попадает в Git.",KP_API_KEY||"");
        if(v===null)return; const key=String(v).trim();
        try{ if(key)localStorage.setItem(KP_API_KEY_STORAGE,key); else localStorage.removeItem(KP_API_KEY_STORAGE); }catch(_){}
        KP_API_KEY=key; label.textContent=key?"КП API: подключен":"КП API: не настроен"; btn.textContent=key?"Сменить API-ключ":"Настроить API-ключ";
        new Notice(key?"API-ключ сохранен. Переоткрой страницу рекомендаций.":"API-ключ удален.");
    };
}
...
async function main(){
    ...
    const ui=makeUi(dv.container),pool=new Map(); addKpApiControl(ui);
    ...
    let sourceDirty=false,kpFirst=0,kpExpanded=0,kpNetworkState="",kpDiag="";
    if(ref.kpId){
        setProgress(ui,"Шаг 3/4 · Кинопоиск: загружаю список похожих по КП ID…",43);await sleep(0);
        const first=await getSimilarKp(ref.kpId,sourceCache);kpNetworkState=first.source;kpDiag=first.diag||"";
        const graph=new Map();
        first.items.forEach((z,i)=>graph.set(z.kpId,{...z,sim:clamp(1-i*0.045,0.62,1),depth:1}));kpFirst=first.items.length;
        if(["kp-api","kinopoisk","movie-planner"].includes(first.source))sourceDirty=true;
        ...
        if(graph.size<20&&first.items.length){
            for(const parent of first.items.slice(0,3)){
                ...
                const sub=await getSimilarKp(parent.kpId,sourceCache);if(["kp-api","kinopoisk","movie-planner"].includes(sub.source))sourceDirty=true;
    ...
    if(kpNetworkState==="kp-api")note+=` · КП-похожие: KP API`;
    else if(kpNetworkState==="kinopoisk")note+=` · КП-похожие: страница Кинопоиска`;
    ...
    else if(kpNetworkState==="kp-api-empty")note+=` · КП-похожие: KP API вернул 0`;
    else if(kpNetworkState&&["kp-blocked","unavailable"].includes(kpNetworkState))note+=` · КП-похожие недоступны`;
    if(kpDiag)note+=` · диагностика: ${kpDiag}`;
