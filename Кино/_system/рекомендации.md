...
async function main(){
    ...
    const draw=()=>{
        const visible=all.filter(x=>checkbox.checked||!x.watched).slice(0,LIMIT);
        renderTable(ui,visible);
        const hidden=all.filter(x=>x.watched).length;
        setProgress(ui,`${seeds.diag} · показано ${visible.length} · просмотренных ${hidden}. MovieLens выключен.`,100);
