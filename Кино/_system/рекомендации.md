...
>
> Уже оцененные тобой произведения исключаются. Для сопоставления с Кинопоиском сначала используется **твоя локальная карточка и КП ID**, затем Wikidata, прямой поиск Кинопоиска и веб-поиск. Movie Planner оставлен только аварийным резервом. **Описание и список похожих сначала берутся через тот же Movie Planner/KP источник, который уже работает в скрипте добавления фильма; прямой сайт Кинопоиска используется только как резерв.** В выдаче - до **20** рекомендаций. Описание берется по найденному КП ID.
...
const KP_TIMEOUT_MS = 12000;
const KP_DETAIL_TIMEOUT_MS = 15000;
    ...
    for(let attempt=0;attempt<2;attempt++){
        ...
        try{
            const r=await withTimeout(http({
                ...
                headers:{Accept:"application/json","User-Agent":"ObsidianKinoteka/3.0 (personal movie catalogue)"}
            }),timeout);
            const status=Number(r?.status||0);
            if(status===200)return {ok:true,status,data:r?.json||null,transient:false};
            if([429,503].includes(status)||status>=500){
                if(attempt===0){await sleep(status===429?2500:1800);continue;}
                return {ok:false,status,data:null,transient:true};
            }
            return {ok:false,status,data:null,transient:false};
        }catch(_){
...
async function kpHttpText(url,timeout=KP_TIMEOUT_MS){
    ...
    for(let attempt=0;attempt<2;attempt++){
        ...
        try{
            ...
            const status=Number(r?.status||0),body=String(r?.text||"");
            if(status===200&&body)return {ok:true,status,text:body,transient:false};
            if([429,503].includes(status)||status>=500){
                if(attempt===0){await sleep(status===429?2500:1800);continue;}
                return {ok:false,status,text:body,transient:true};
            }
            return {ok:false,status,text:body,transient:false};
        }catch(_){
            if(attempt===0){await sleep(1200);continue;}
...
function parseSimilarHtml(html,refId){
    ...
    try{
        ...
        const h=hs.find(x=>/похожие(?:\s+на)?/i.test(String(x.textContent||"")));
        if(h){
            let root=h.parentElement,up=0;
            while(root&&up++<5){
                const links=[...root.querySelectorAll('a[href*="/f/"]')];
                if(links.length>=3){for(const a of links)add(a.getAttribute("href")||a.href,a.textContent);break;}
                root=root.parentElement;
            }
            if(found.length<3){
                ...
                while(n&&guard++<16){
                    ...
                    const links=n.querySelectorAll?[...n.querySelectorAll('a[href*="/f/"]')]:[];
                    for(const a of links)add(a.getAttribute("href")||a.href,a.textContent);
    ...
    if(found.length<3){
        ...
        const i=raw.search(/Похожие(?:\s+на)?/i);
        const seg=i>=0?raw.slice(i,i+120000):raw;
...
function parseMoviePlannerPage(html,kpId,fallbackTitle=""){
    if(!html)return null;
    let ruTitle="",year="",description="",kpRating=null;
    try{
        const doc=new DOMParser().parseFromString(String(html),"text/html");
        const h1=doc.querySelector("h1");
        const h1t=String(h1?.textContent||"").replace(/\s+/g," ").trim();
        const ym=h1t.match(/\((?:18|19|20)\d{2}\)\s*$/);
        year=ym?ym[0].replace(/\D/g,""):"";
        ruTitle=h1t.replace(/\s*\((?:18|19|20)\d{2}\)\s*$/,"").trim();

        const heads=[...doc.querySelectorAll("h2,h3,h4")];
        const descHead=heads.find(x=>/описание и сюжет/i.test(String(x.textContent||"")));
        if(descHead){
            let n=descHead.nextElementSibling,guard=0;
            while(n&&guard++<10){
                if(/^H[234]$/.test(n.tagName))break;
                const t=cleanPlotText(n.textContent||"");
                if(goodPlot(t)&&t.length>(description?.length||0))description=t;
        ...
        if(!description){
            for(const sel of ['meta[property="og:description"]','meta[name="description"]']){
                const t=cleanPlotText(doc.querySelector(sel)?.getAttribute("content")||"");
                if(goodPlot(t)){description=t;break;}
            }
        }
        const body=String(doc.body?.textContent||"").replace(/\s+/g," ");
        const rm=body.match(/(?:Рейтинг\s+Кинопоиска|КП)\s*([0-9]+(?:[.,][0-9]+)?)/i);
        if(rm)kpRating=num(rm[1]);
    }catch(_){}

    if(!description){
        const raw=String(html);
        const i=raw.search(/описание и сюжет/i);
        if(i>=0){
            const seg=decodeHtmlText(raw.slice(i,i+18000));
            const m=seg.match(/описание и сюжет\s+(.{70,2200}?)(?=\s+(?:Жанр|Страна|Рейтинг Кинопоиска|Актёры|Создатели|Похожие|В Movie Planner))/i);
            if(m&&goodPlot(m[1]))description=cleanPlotText(m[1]);
        }
    }
    return {
        kpId:String(kpId||"").replace(/\D/g,""),
        ruTitle:ruTitle||fallbackTitle||"",
        enTitle:"",
        year,
        description:description||"",
        kpRating,
        source:"movie-planner-page"
    };
}
async function getSimilarKp(kpId,sourceCache){
    ...
    if(old&&Date.now()-Number(old.at||0)<SIMILAR_TTL&&Array.isArray(old.items)&&old.items.length){
        return {items:old.items,source:"cache"};
    }

    const mp=await kpHttpText(`https://movie-planner.ru/f/${kpId}`,12000);
    if(mp.ok){
    ...
    if(kp.ok){
        ...
        if(items.length){
            sourceCache.similar[kpId]={at:Date.now(),items,source:"kinopoisk"};
            return {items,source:"kinopoisk"};
        }
    }

    if(old?.items?.length)return {items:old.items,source:"cache-stale"};
    return {items:[],source:(mp.transient||kp.blocked)?"temporarily-unavailable":"empty"};
}
...
async function getKpDetail(kpId,fallbackTitle,sourceCache){
    ...
    const api=await kpHttpJson(`https://movie-planner.ru/api/public/film/${kpId}`,15000);
    if(api.ok){
        const d=parseKpDetail(api.data,kpId,fallbackTitle);
        if(d){
            sourceCache.details[kpId]={at:Date.now(),data:d,source:d.description?"movie-planner-api":"movie-planner-api-partial"};
    ...
    const mp=await kpHttpText(`https://movie-planner.ru/f/${kpId}`,12000);
    if(mp.ok){
        const d=parseMoviePlannerPage(mp.text,kpId,fallbackTitle);
        const previous=sourceCache.details[kpId]?.data||old?.data||{};
        const merged={...previous,...d,
            ruTitle:d?.ruTitle||previous?.ruTitle||fallbackTitle||"",
            description:d?.description||previous?.description||"",
            kpRating:d?.kpRating??previous?.kpRating??null
        };
        sourceCache.details[kpId]={at:Date.now(),data:merged,source:"movie-planner-page"};
        if(merged.description)return merged;
    }
    ...
    if(kp.ok){
        ...
        sourceCache.details[kpId]={at:Date.now(),data:merged,source:"kinopoisk"};
        if(merged.description)return merged;
    }

    return sourceCache.details[kpId]?.data || old?.data || null;
}
...
async function main(){
    ...
    if(kpNetworkState&&["temporarily-unavailable","empty","kp-blocked","unavailable"].includes(kpNetworkState))note+=` · внешний список похожих сейчас недоступен, MovieLens продолжил работу`;
