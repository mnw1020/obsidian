...
async function main(){
    ...
    const draw=()=>{
        const visible=combineRecommendationLists(all,(imdbCheckbox.checked?imdbRows:[]).concat(mlCheckbox.checked?mlRows:[]),checkbox.checked);
        ...
        setProgress(ui,`${seeds.diag} · показано ${visible.length} · просмотренных ${hidden}. IMDb ${imdbStatus}; MovieLens ${mlStatus}.`,100);
