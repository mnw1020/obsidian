...
async function getSimilarKp(kpId,sourceCache){
    ...
    // 1) Direct Kinopoisk page - this is now the primary source.
    ...
    // 2) Keep a previously working cache even if Kinopoisk is temporarily unavailable.
    if(old?.items?.length)return {items:old.items,source:"cache-stale"};

    // 3) Movie Planner is only a fallback, never a hard dependency.
    const mp=await kpHttpText(`https://movie-planner.ru/f/${kpId}`,2200);
    if(mp.ok){
        const items=parseSimilarHtml(mp.text,kpId);
        if(items.length){sourceCache.similar[kpId]={at:Date.now(),items,source:"movie-planner"};return {items,source:"movie-planner"};}
    }
...
async function main(){
    ...
    if(kpNetworkState&&["kp-blocked","unavailable"].includes(kpNetworkState))note+=` · КП-похожие сейчас недоступны, MovieLens продолжил работу`;
