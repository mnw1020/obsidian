...
async function main(){
    ...
    const imdbLabel=ui.actions.createEl("label");
    const imdbCheckbox=imdbLabel.createEl("input",{type:"checkbox"});
    imdbLabel.appendText(" IMDb — More like this");
    ...
    const draw=()=>{
        const visible=combineRecommendationLists(all,(tmdbCheckbox.checked?tmdbRows:[]).concat(imdbCheckbox.checked?imdbRows:[]).concat(mlCheckbox.checked?mlRows:[]),checkbox.checked);
        ...
        const imdbStatus=!imdbCheckbox.checked?"выключен":imdbError?imdbError:!imdbReady?"загружается…":`включён (${imdbRows.length})`;
        const tmdbStatus=!tmdbCheckbox.checked?"выключен":tmdbError?tmdbError:!tmdbReady?"загружается…":`включён (${tmdbRows.length})`;
        setProgress(ui,`${seeds.diag} · показано ${visible.length} · просмотренных ${hidden}. TMDB ${tmdbStatus}; IMDb ${imdbStatus}; MovieLens ${mlStatus}.`,100);
    ...
    imdbCheckbox.addEventListener("change",async()=>{
        if(imdbCheckbox.checked&&!imdbReady){const r=await imdbSimilarRecommendations(ref,items);imdbRows=r.items;imdbError=r.diag;imdbReady=true;}
        draw();
    });
