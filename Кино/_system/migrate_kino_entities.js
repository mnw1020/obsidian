...
const patchedActorPage = SERVICE_PAGES["Кино/_system/Актер.md"]
  .replace(
    'const selected = String(dv.current()["Выбрано"] || "");',
    `function kinoPersonName(value) {
...
function kinoPersonKeys(value) {
    const text = kinoPersonName(value);
    const pair = text.match(/^(.+?)\\s*\\(([^()]*)\\)$/);
    const parts = pair ? [pair[1], pair[2]] : [text];
    return new Set(parts.map(kinoPersonKey).filter(Boolean));
}
function kinoSamePerson(left, right) {
    const leftKeys = kinoPersonKeys(left);
    return [...kinoPersonKeys(right)].some(key => leftKeys.has(key));
}
...
SERVICE_PAGES["Кино/_system/Актер.md"] = [
  ...
  "  dv.header(2, selected);",
  "  const rows = dv.pages('\"Кино/_system/Роли\"').array().map(rolePage => { const path = String(rolePage[\"Основная карточка\"]?.path || rolePage[\"Основная карточка\"] || \"\").replace(/^\\[\\[|\\]\\]$/g, \"\").replace(/\\.md$/, \"\"); return { rolePage, movie: path ? dv.page(path) : null }; }).filter(({ rolePage, movie }) => { const values = Array.isArray(rolePage[\"Актеры\"]) ? rolePage[\"Актеры\"] : [rolePage[\"Актеры\"]]; return movie && values.some(value => personKey(value) === personKey(selected)); });",
...
const COMMANDS = [
  ...
  {
    "id": "kinoteka-update-actor-roles-v1",
    "name": "Кино - Обновить роли актёров",
    "type": "UserScript",
    "command": true,
    "path": "скрипты/update_kino_roles.js",
    "settings": {}
  }
];
const ENTITY_META = {
