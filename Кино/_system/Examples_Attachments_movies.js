...
const KINO_PERSON_CACHE_PATH = "Кино/_system/kino-person-alias-cache.json";
let KINO_PERSON_DYNAMIC_ALIASES = {Актеры: {}, Режисер: {}};
async function kinoLoadPersonCache(app) {
    const file = app.vault.getAbstractFileByPath(KINO_PERSON_CACHE_PATH);
    if (!file) return;
    try {
        const data = JSON.parse(await app.vault.read(file));
        KINO_PERSON_DYNAMIC_ALIASES = {
...
async function addMovie(params, settings) {
    ...
    params.variables = {
        ...
        actorLinks: linkList(movie.Actors, "Актеры"),
        genreLinks: linkList(movie.Genre, "Жанр"),
        directorLink: linkList(movie.Director, "Режисер"),
        fileName: title,
...
function links(value, field) {
    return [...new Set(clean(value).split(",").map(x => kinoPersonDisplay(field, entityName(x))).filter(Boolean))];
}
