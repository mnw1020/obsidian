...
// Резервные связи IMDb -> КП, если автоматический поиск не вернул ID.
// Роли всё равно запрашиваются сначала у IMDb, КП используется только как fallback.
...
async function kinopoisk(get, qa, movie, wiki) {
    ...
    const knownKpId = KNOWN_KP_BY_IMDB[extractImdbId(movie?.imdbID)];
    if (knownKpId) {
        const known = await kinopoiskById(get, knownKpId);
