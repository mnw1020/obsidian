...
const KINO_IMDB_KP_CACHE_PATH = "Кино/_system/kino-imdb-kp-cache.json";
let KINO_PERSON_DYNAMIC_ALIASES = {Актеры: {}, Режисер: {}};
let KINO_IMDB_KP_CACHE = {};
...
async function kinoLoadImdbKpCache(app) {
    const file = app.vault.getAbstractFileByPath(KINO_IMDB_KP_CACHE_PATH);
    if (!file) return;
    try {
        const data = JSON.parse(await app.vault.read(file));
        KINO_IMDB_KP_CACHE = data && typeof data === "object" ? data : {};
    } catch (_) {
        KINO_IMDB_KP_CACHE = {};
    }
}
async function kinoSaveImdbKpCache(app, imdbId, kpId) {
    if (!/^tt\d{7,12}$/i.test(String(imdbId || "")) || !/^\d{4,12}$/.test(String(kpId || ""))) return;
    KINO_IMDB_KP_CACHE[String(imdbId).toLowerCase()] = String(kpId);
    const text = JSON.stringify(KINO_IMDB_KP_CACHE, null, 2) + "\n";
    try {
        const file = app.vault.getAbstractFileByPath(KINO_IMDB_KP_CACHE_PATH);
        if (file) await app.vault.modify(file, text);
        else await app.vault.create(KINO_IMDB_KP_CACHE_PATH, text);
    } catch (_) {}
}
...
async function addMovieCore(params, settings, progress, handOff) {
    ...
    if (!movie || movie.Response === "False" || !/^tt\d{7,12}$/.test(movie.imdbID || "")) {
        ...
        movie = movieFromKinopoisk(kp, imdbId);
    }
...
function personMatchKeys(value) {
    ...
    return new Set([
        ...
        transliterateRussian(text).replace(/[^0-9a-z]/gi, "")
    ].filter(Boolean));
}
...
async function kinopoisk(get, qa, movie, wiki) {
    ...
    const cachedId = KINO_IMDB_KP_CACHE[String(movie.imdbID || "").toLowerCase()];
    if (cachedId) {
        const cached = await kinopoiskById(get, cachedId);
