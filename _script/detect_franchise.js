/*
 * QuickAdd: поиск франшизы. Последний шаг ПОСЛЕ Capture/Template или отдельный макрос.
...
let running = false;
module.exports = async function detectFranchise(params) {
    if (running) return;
    running = true;
    try { await editFranchise(params); }
    finally { running = false; }
};

async function editFranchise(params) {
    const { app, quickAddApi, obsidian } = params;
    const notify = message => new obsidian.Notice(message, 5000);
    const cached = file => fresh.get(file.path) ?? app.metadataCache.getFileCache(file)?.frontmatter ?? {};
    const allFiles = app.vault.getMarkdownFiles();
    // После Capture индекс может ещё не обновиться. Читаем YAML с диска.
    const fresh = new Map();
    for (const item of allFiles.filter(f => f.path.startsWith(MEDIA_FOLDER + "/"))) {
        fresh.set(item.path, await readFrontmatter(item));
    }
    ...
    const macroId = imdbId(params.variables?.imdbID);
    const active = macroId ? null : app.workspace.getActiveFile();
    let file;
    if (macroId) {
        const matches = media.filter(item => imdbId(cached(item)["imdb Id"]) === macroId);
        if (!matches.length) {
            notify("Сначала создай карточку фильма. Этот шаг должен стоять после Capture / Template.");
            return;
        }
        file = matches.length === 1 ? matches[0] : await quickAddApi.suggester(
            matches.map(item => item.path), matches, "Какую карточку связать?"
        );
        if (!file) return;
    } else file = media.find(item => item.path === active?.path);
    ...
    const id = imdbId(before["imdb Id"]);
    const suggestions = await detectSeries(id, obsidian);
    const suggested = suggestions.map(suggestion => {
        const path = `${FRANCHISE_FOLDER}/${safeName(suggestion.name)}.md`;
        const existing = list.find(item => item.path.toLowerCase() === path.toLowerCase());
        return { ...(existing || { name: suggestion.name, path, create: true }), suggestion };
    });
    const create = { create: true };
    const skip = { skip: true };
    let choice = await quickAddApi.suggester(
        [...suggested.map(item => `Источник: ${item.name} (${item.suggestion.origin})`),
         ...list.map(item => `${item.name} - ${item.path}`), "+ Новая франшиза", "Оставить без изменений"],
        [...suggested, ...list, create, skip],
        suggestions.length ? `${file.basename}: выбери основную франшизу` : `${file.basename}: выбери франшизу вручную или пропусти`
    );
    if (choice?.skip) return;
    if (choice?.suggestion) {
        const action = await quickAddApi.suggester(
            ["Использовать эту связь", "Открыть источник", "Выбрать существующую страницу", "Пропустить"],
            ["use", "source", "existing", "skip"],
            `${file.basename} → ${choice.name}`
        );
        if (!action || action === "skip") return;
        if (action === "source") {
            globalThis.open?.(choice.suggestion.url);
            return;
        }
        if (action === "existing") {
            const suggestion = choice.suggestion;
            choice = await quickAddApi.suggester(list.map(item => item.name), list, "Выбери страницу этой франшизы");
            if (choice) choice = { ...choice, suggestion };
        }
    }
    if (!choice) return;

    if (choice.create) {
        const input = await quickAddApi.inputPrompt("Название франшизы", "Например: Трансформеры", choice.suggestion?.name || "");
        ...
        choice = { name: existing?.basename || name, path: existing?.path || path, file: existing, suggestion: choice.suggestion };
    }
    ...
    // Источник сохраняем рядом со связью, без изменения текста фильма.
    if (choice.suggestion) {
        const marker = `<!-- FRANCHISE:SOURCE:${id} -->`;
        const source = choice.suggestion;
        await app.vault.process(page, raw => raw.includes(marker) ? raw : raw.trimEnd() +
            `\n\n${marker}\nИсточник связи для IMDb ${id}: [${source.origin}](${source.url}).\n`);
    }
...
// Один последовательный запрос; повтор только при 429/503, с Retry-After.
// Кэш на время сеанса: сутки для найденного, 5 минут для пустого ответа.
...
function imdbId(value) {
    ...
    return /^tt\d{7,12}$/.test(id) ? id : "";
}

function parseSeries(payload) {
    const rows = payload?.results?.bindings;
    if (!Array.isArray(rows) || rows.length >= 100) return [];
    // Два разных произведения с одним IMDb ID - не выбираем первое.
    const items = new Set(rows.map(row => row.item?.value).filter(Boolean));
    if (items.size !== 1) return [];
    const item = [...items][0];
    if (!/^https?:\/\/www\.wikidata\.org\/entity\/Q\d+$/.test(item)) return [];
    const result = new Map();
    for (const row of rows) {
        const uri = row.series?.value || "";
        if (!/^https?:\/\/www\.wikidata\.org\/entity\/Q\d+$/.test(uri)) continue;
        const qid = uri.split("/").pop();
        const name = row.seriesLabel?.value;
        if (!name || name === qid) continue;
        result.set(qid, { name, qid, origin: "Wikidata, связь P179",
            url: `https://www.wikidata.org/wiki/${item.split("/").pop()}#P179` });
    }
    return [...result.values()];
}

function retryMilliseconds(headers, now = Date.now()) {
    ...
    return Number.isFinite(date) ? Math.max(1000, date - now) : 5000;
}

async function detectSeries(id, obsidian) {
    if (!imdbId(id)) return [];
    if (CATALOG[id]) return CATALOG[id];
    const hit = onlineCache.get(id);
    if (hit && hit.expires > Date.now()) return hit.value;
    if (nextRequestAt - Date.now() > 15000) return [];
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
            const response = await Promise.race([
                obsidian.requestUrl({ url, method: "GET", throw: false,
                    headers: { Accept: "application/sparql-results+json",
                        "User-Agent": "ObsidianKinotekaFranchises/1.0 (personal movie catalogue)" } }),
                new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("timeout")), 15000); })
            ]);
            if ([429, 503].includes(response.status)) {
                const wait = retryMilliseconds(response.headers);
                nextRequestAt = Date.now() + wait;
                if (attempt === 0 && wait <= 15000) continue;
                return [];
            }
            if (response.status !== 200) return [];
            const value = parseSeries(response.json);
            onlineCache.set(id, { value, expires: Date.now() + (value.length ? 86400000 : 300000) });
            return value;
        } catch {
            // Не продолжаем запросы после таймаута: предыдущий ещё может выполняться.
